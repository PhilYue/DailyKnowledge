...
Vue.http.interceptors.push(function (req) {
  ...
  return function (res) {
    if (res.status !== 200) {
      Message.error('服务当前不可用。')
    } else if (res.body.code !== 200) {
      console.error(res.url, res.body.message)
    }
  }
})
...
Vue.use(ElementUI)
Vue.mixin(mixins)
const apiUrl = 'http://116.62.166.134:'
const apiUrl31 = 'http://116.62.166.134:'
const store = new Vuex.Store({
  state: {
    ...
    // workflowToken: '1',
    // rent服务接口地址
    rentUrl: apiUrl + '23150/rent/',
    // house房源接口地址
    houseUrl: apiUrl + '23222/house/',
    // contract合同接口地址
    contractUrl: apiUrl + '23250/contract/',
    // account登录接口地址
    accountUrl: apiUrl + '21000/account/',
    // pdf接口地址
    pdfUrl: apiUrl + '21350/oss/',
    // pay接口地址
    payUrl: apiUrl31 + '21550/pay/',
    // 工作流接口地址
    workflowUrl: apiUrl + '21600/workflow/',
    // 售后服务接口地址
    serviceUrl: apiUrl + '23300/maintain/',
    // 搜索引擎接口地址
    searchUrl: apiUrl + '21275/search/',
    // 维修物品接口地址
    categoryUrl: apiUrl + '21275/data/',
    // 三级联动接口地址
    blockUrl: apiUrl + '21275/block/',
    // 同步房源数据到蘑菇租房
    alipayUrl: apiUrl + '21650/house/sync/',
    // 智能门锁
    lockUrl: apiUrl31 + '21151/lock/',
    adminleftnavnum: '',
    houseleftnavnum: '',
    topIndex: '',
    menuleftnavnum: '',
