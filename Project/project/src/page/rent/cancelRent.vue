            ...
            <!-- 列表组件 -->
              <lease-list :listData="listData">
                <template slot-scope="props" class="pad9">
                  ...
                  <!-- 退租待确认的按钮 -->
                    <el-dropdown v-if="listData[props.index].leaseStatus === 'WATHROW'" menu-align="start">
                      <el-button>
                        <span>操作</span>
...
export default {
  ...
  data: function () {
    return {
      ...
      // 退租数据
      rents: ['全部', '退租待确认', '退租费用未结清', '退租费用已结清', '已退租'],
      activeName: '全部',
      input: '',
      // 列表数据
      listData: [],
  ...
  methods: {
    ...
    selected (rent) {
      ...
      if (rent === '全部') {
        this.bean.leaseStatus = 'THROW'
      } else if (rent === '退租待确认') {
        this.bean.leaseStatus = 'WATHROW'
      } else if (rent === '退租费用未结清') {
        this.bean.leaseStatus = 'NSTHROW'
      } else if (rent === '退租费用已结清') {
        this.bean.leaseStatus = 'ASTHROW'
      } else if (rent === '已退租') {
        this.bean.leaseStatus = 'THEND'
      }
      this.loadData(this.bean)
    },
    // 当前页码改变触发
  ...
  created () {
    ...
    // 加载出第一次加载的城市
    this.loadFirst()
    this.$store.state.adminleftnavnum = '/rent/cancelRent'
