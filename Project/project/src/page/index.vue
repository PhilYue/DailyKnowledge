       ...
       <!-- 统计的数据 -->
          ...
          <el-tabs :style="heigh" type="border-card" @tab-click="tabClick">
            <el-tab-pane label="待办任务">
              ...
              <area-select @sendUnitId="getTaskUnitId" :areaData="taskareaData"></area-select>
            ...
            <el-tab-pane label="应收账单">
              ...
              <div class="main-right-search-span">
                <a :class="{selectActive: activeName == '全部'}" @click="selected('全部')">全部</a>
                <a :class="{selectActive: activeName == '未收款'}" @click="selected('未收款')">未收款</a>
                <span :class="{selectActive: activeName == '支付成功'}" @click="selected('支付成功')">支付成功</span>
                <span :class="{selectActive: activeName == '(线下支付'}" @click="selected('(线下支付')">(线下支付</span>
                <a :class="{selectActive: activeName == '线上支付)'}" @click="selected('线上支付)')">线上支付)</a>
                <a :class="{selectActive: activeName == '财务已确认'}" @click="selected('财务已确认')">财务已确认</a>
              </div>
              ...
              <!-- 费用详情组件 -->
              <pay-detail :detailShow="detailShow" :payData="payData"></pay-detail>
              <!-- 分拆费用弹窗 -->
...
import CollectList from '@/components/pay/CollectList' // 应收列表组件
import PayDetail from '@/components/pay/PayDetail' // 费用详情组件
import PayList from '@/components/pay/PayList' // 应付账单组件
import PayRentList from '@/components/pay/PayRentList' // 退租账单组件
  ...
  methods: {
    ...
    selected2 (rent) {
      ...
      if (rent === '全部') {
        ...
        this.taskBean.taskType = 'PROCESS-KEY-RENT-V11'
      } else if (rent === '换租流程') {
    ...
    selected (rent) {
      ...
      if (rent === '全部') {
        ...
        this.bean.orderPhase = 'SIMPLE_PAID'
        this.bean.orderStatus = null
      } else if (rent === '线上支付)') {
        this.selectedShow = true
        this.bean.orderPhase = null
        this.bean.orderStatus = 'SUC'
      } else if (rent === '(线下支付') {
        this.selectedShow = true
        this.bean.orderPhase = null
        this.bean.orderStatus = 'PDOFF'
      } else if (rent === '财务已确认') {
        this.selectedShow = true
        this.bean.orderPhase = null
