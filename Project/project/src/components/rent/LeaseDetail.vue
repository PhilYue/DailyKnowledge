  ...
  <div>
    <transition name="fade">
      <div v-if="detailShow.show" class="dialog" :style="height">
            ...
            <div class="contain">
              <div class="clear">
                ...
                <el-col v-if="detailData.documentsType" :span="16">
                  ...
                  <div v-else-if="detailData.documentsType === 'BL'" class="content">营业执照</div>
              ...
              <div class="clear">
                ...
                <!-- 退租原因 -->
                <el-col v-if="detailData.throwType" :span="8" class="title">退租类型</el-col>
                <el-col v-if="detailData.throwType" :span="16">
                  <div class="content titleF">{{detailData.throwType | throwTypeFilter}}</div>
                </el-col>
                ...
                <el-col v-if="detailData.settleMemo" :span="8" class="title">费用备注</el-col>
                <el-col v-if="detailData.settleMemo" :span="16">
                  <div class="content">{{detailData.settleMemo}}</div>
                </el-col>
              </div>
              <div class="clear">
                <h3>计费规则</h3>
                <div :key="index" v-for="(item,index) in detailData.chargList">
        ...
        <div @click="closeLease" class="el-icon-close closeIco"></div>
      </div>
    </transition>
    <business-detail :detailShow="detailShowBusiness" :businessId="businessId"></business-detail>
  </div>
</template>
<script>
const height = window.screen.height - 51
// 租约详情组件
import BusinessDetail from '@/components/rent/BusinessDetail'
import $ from 'jquery'
export default {
  name: 'detail',
  data: function () {
  ...
  methods: {
    ...
    closeLease: function () {
      this.detailShow.show = !this.detailShow.show
      this.$emit('selectIndex', 1)
    }
  },
  components: {
    BusinessDetail
  ...
  filters: {
    throwTypeFilter: function (value) {
      if (value === 'NORMAL') {
        return '正常退租'
      } else if (value === 'ISRESP') {
        return '有责提前退租'
      } else if (value === 'NORESP') {
        return '无责结算退租'
      } else if (value === 'NSETTLE') {
        return '无需结算退租'
      }
    }
  },
