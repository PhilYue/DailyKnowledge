<template>
  <div class="index">
    ...
    <!-- 退租转租筛选 -->
    <el-dropdown menu-align="start" class="el-dropdown" @command="handleContractType">
      <span class="el-dropdown-link houseRentType">
        {{contractType}}<i class="el-icon-caret-bottom el-icon--right"></i>
      </span>
      <el-dropdown-menu slot="dropdown">
        <el-dropdown-item command="全部">全部</el-dropdown-item>
        <el-dropdown-item command="退租结算">退租结算</el-dropdown-item>
        <el-dropdown-item command="转租结算">转租结算</el-dropdown-item>
        <el-dropdown-item command="换房结算">换房结算</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
    ...
    <div class="list-contain-main">
      <collect-list :listData="listData" :yqShow="yqShow">
        <template slot-scope="props" class="pad9">
          <el-col v-if="listData[props.index].orderStatus === 'NEW' || listData[props.index].orderStatus === 'RETURNING'" :span="2" class="pad9">
            <el-dropdown menu-align="start">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item v-show="isRole && !isGudong || isRole2" @click.native="change(props)">修改金额</el-dropdown-item>
    ...
    <!-- 预定生成费用详情 -->
    <pay-detail :detailShow="detailShow" :detailData="detailData" :payData="payData"></pay-detail>
  </div>
</template>
<script>
import $ from 'jquery'
...
export default {
  ...
  methods: {
    ...
    // 结算类型筛选
    handleContractType (command) {
      console.log(command)
      this.contractType = command
      if (command === '全部') {
        this.bean.createReasonType = null
      } else if (command === '转租结算') {
        this.bean.createReasonType = 'SUBLET'
      } else if (command === '退租结算') {
        this.bean.createReasonType = 'CANCEL'
      } else if (command === '换房结算') {
        this.bean.createReasonType = 'CHANGE'
      }
      this.loadData(this.bean)
    },
    // 当前页码改变触发
    ...
    selected (rent) {
      ...
      if (rent === '全部') {
        ...
        this.selectedShow = true
        this.bean.orderPhase = 'RETURN_UN_PAID'
        this.bean.orderStatus = null
      } else if (rent === '已制单') {
        this.selectedShow = true
    ...
    searchTime () {
      if (this.minTime || this.maxTime) {
        this.bean.moveOutTimeStartInterval = Date.parse(this.minTime)
        this.bean.moveOutTimeEndInterval = Date.parse(this.maxTime)
        this.loadData(this.bean)
      } else {
        this.bean.moveOutTimeStartInterval = null
        this.bean.moveOutTimeEndInterval = null
      }
    },
    ...
    // 修改金额方法
    change (props) {
      this.$prompt('请修改金额', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPattern: '',
        inputErrorMessage: '',
        inputPlaceholder: '请输入金额数目',
        inputValue: ''
      }).then(({ value }) => {
        // 改变状态为线下已收款
        this.$http.post(this.$store.state.payUrl + 'update', {
    ...
    confirm (props) {
      let orderStatus
      if (this.listData[props.index].financeType === 'DESP') {
        orderStatus = 'RET'
      } else {
        orderStatus = 'CONFIRM'
      }
    ...
    submit () { // 刷新列表
      this.$options.methods.loadTaskData(this)
    },
    loadTaskData (_this) {
       // 请求待办任务的数据
  ...
  created () {
    ...
    this.$root.task.$on('submitForm', value => {
      // this.loadTaskData(this)
      setTimeout(this.loadTaskData(this), 1000) // 加载任务列表
    })
  },
  mounted: function () {
    if ($this.int === 0) {
