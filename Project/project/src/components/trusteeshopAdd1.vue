...
<!-- 新建托管合同 -->
  <div class="trustshipAdd">
    <el-tabs v-model="activeName2" type="card" @tab-click="handleClick">
      <el-tab-pane label="分散式合同" name="first">
        <el-form :model="form" :inline="true" style="width:650px; margin:0 auto;">
          ...
          <!--  <el-form-item label="违约金" :label-width="formLabelWidth" class="is-required">
            ...
            <el-form-item label="付款周期" :label-width="formLabelWidth">
              <el-select v-model="form.cycle" placeholder="请选择" style="width:169px;">
            ...
            <div class="form-item-group">
              <div :key="index" v-for="(item, index) in payArr">
                <!-- 费用列表循环 -->
                <el-row>
                  <el-col :span="12">
                    <el-form-item label="费用开始时间" label-width="7em">
                      <el-date-picker type="date" placeholder="请选择费用开始时间" v-model="item.startTime"></el-date-picker>
                    </el-form-item>
                  </el-col>
                  <el-col :span="12">
                    <el-form-item label="费用结束时间" label-width="7em">
                ...
                <el-row>
                  <el-col :span="12">
                    <el-form-item label="费用金额" label-width="7em">
                      <el-input v-model="item.money" placeholder="请输入费用金额"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="12">
                    <el-form-item label="收费提前天数" label-width="7em">
                ...
                <div class="_btn-wrap">
                  <img v-if="payArr.length > 1" @click="removePay(index)" class="_btn" src="../../static/img/delete.png">
                </div>
              </div>
              
              <div class="_btn-wrap">
                <img @click="addPay" class="_btn" src="../../static/img/add.png">
              </div>
            </div>
          ...
          <el-form-item label="纸质合同编号" :label-width="formLabelWidth"  class="is-required">
            <el-input v-model="form.paperNum" placeholder="纸质合同编号"/>
          </el-form-item>
          <el-form-item label="签约时间" :label-width="formLabelWidth"  class="is-required">
            <el-date-picker v-model="form.signTime" placeholder="签约时间"/>
          </el-form-item>
      ...
      <el-tab-pane label="集中式合同" name="second">
...
export default {
  ...
  methods: {
    ...
    validate530NewAdd() {
      if (!this.form.paperNum) {
        this.$message.error('纸质合同编号不能为空。')
        return false
      }
      
      if (!this.form.signTime) {
        this.$message.error('签约时间不能为空。')
        return false
      }

      let find = this.form.reqChargInputLogBeanList.find(o => {
        console.log('o', o)
        return !Object.values(o).every(Boolean)
      })
            ...
            if (valueD.body.code === 200) {
              ...
              this.$http.post(houseUrl, {
                ...
                if (res.body.code === 200) {
                  ...
                  data1.houseType = 'DISTRIBUTE'
                  data1.housePrice = data1.rental1 * 1000
                  const reqChargInputLogBeanList = () => {
                    this.form.reqChargInputLogBeanList = []
                    // 每期费用数组
                  ...
                  if (!this.validate530NewAdd()) return
                  ...
                  this.$message({
                    message: `房源注册问题，原因：${res.body.message}`,
                    type: 'error'
                  })
                  this.clickOver = true
                }
    ...
    openAddType (value) { // 打开不规则递增
      ...
      if (value === '3') {
        if (isNaN(effectTime)) {
          ...
          this.endTime = this.moment(this.form.startTime)
            .add(this.form.year, 'years')
            .add(this.form.month, 'months')
            .add(this.form.day, 'days')
            .format()
...
.form-item-group ._btn-wrap {
  text-align: center;
}
.form-item-group ._btn {
  width: 32px;
  height: 32px;
  margin-bottom: 8px;
  display: inline-block;
