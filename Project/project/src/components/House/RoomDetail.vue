...
<template>
  <div class="houseDetail" :style="{zIndex: zIndex}">
    <div class="u-mask" v-if="visible_" @click="handleMaskClick"></div>

    <transition name="fade">
      <div v-if="visible_" class="dialog" :style="height" style="width:600px" >
...
export default {
  ...
  // props: ['houseSpaceId', 'clickCount', 'detailShowPage', 'detailHouseType', 'visible', 'houseDetailText', 'zIndex', 'houseEditWay'],
  props: {
    houseSpaceId: String,
    detailShowPage: String,
    visible: Boolean,
    zIndex: Number,
  },
  watch: {
  ...
  methods: {
    ...
    conStatus (data) { // 转换合同状态以及合同生效时间以及结束时间
      ...
      for (var i = 0; i < len; i++) {
        ...
        data[i].contractStatusChange = contractDict[pay] || ''
