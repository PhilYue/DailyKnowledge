...
export default {
  ...
  methods: {
    ...
    frozenPwd: function (item) {
      ...
      if (that.$store.state.currentUser.roleData.indexOf('小管家') !== -1 || that.$store.state.currentUser.roleData.indexOf('运营部经理') !== -1 || that.$store.state.currentUser.roleData.indexOf('运营专员') !== -1 || that.$store.state.currentUser.roleData.indexOf('运营主管') !== -1) {
        dingtalk.ready(function(){
          const dd = dingtalk.apis;
