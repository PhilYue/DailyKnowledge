<template>
  <div class="houseManager">
    ...
    <div  class="main-right yxr-ui" >
        ...
        <el-tab-pane label="合租" name="first">
          <div class='search f-cb'>
            ...
            <div class='main-left-search-span'>
              <el-input placeholder="请输入房源地址" auto-complete="off" style="width:300px" v-model="searchList.keywordAddress" v-on:keyup.enter.native="houseSearch"></el-input>
              <el-button type='primary' @click="houseSearch">搜索</el-button>
            </div>
          </div>
          <!-- <el-button v-if="houseButtonShow && totlePage > 0" type="primary" class="house_button" @click="slideDown">展开全部列表</el-button> -->
          <!-- <el-button v-if="!houseButtonShow && totlePage > 0" type="primary" class="house_button" @click="slideUp">收起全部房源</el-button> -->
          <!-- <el-button type="primary" class="house_button" @click="slideUp">显示全部房源</el-button> -->
          <houseCard
            :houseLists="flatShareData"
            :elTabPane="elTabPane"
      ...
      <!-- <ul>
        <li class="houseList" name="合租">
        </li>
        <li class="houseList" name="整租" style="display:none">
        </li>
        <li class="houseList" name="集中式" style="display:none">
        </li>
      </ul> -->
      <!-- 房源详情开始 -->
      <houseDetail
...
export default {
  ...
  components: {
    houseCard, houseCardList, houseDetail, depositEdit, houseCard1, houseCard2, houseEdit, userDetail, rentalDetail, goodAdd, reduceGood, conDetail, contractForm, upload, housePassword, contractDetail, areaSelect
  },
  mounted: function () {
    if ($this.int === 0) {
      ...
      $('.newStatus').click(function (event) {
        ...
        let label = ''
        // 选中的部分添加cur样式
        if (event.currentTarget.className.indexOf('cur') < 0) {
          if ($this.elTabPane === 0) {
          ...
          event.currentTarget.className = 'newStatus cur'
          if (event.currentTarget.localName === 'a') {
            // console.log(event)
            status = event.currentTarget.id
          }
          if (event.currentTarget.localName === 'p') {
            status = event.currentTarget.id
          }
          if (event.currentTarget.localName === 'li') {
            status = event.currentTarget.id
            label = event.currentTarget.textContent
            event.currentTarget.parentElement.previousElementSibling.previousElementSibling.className = 'newStatus cur'
            event.currentTarget.parentElement.previousElementSibling.previousElementSibling.id = status
            event.currentTarget.parentElement.previousElementSibling.previousElementSibling.innerText = label
          }
        }
