            ...
            <!-- 列表组件 -->
              <leaseList :listData="listData">
                <template slot-scope="props" class="pad9">
                  ...
                  <!-- 转租待确认显示的按钮 -->
                    <el-dropdown v-if="listData[props.index].leaseStatus === 'WASUBLET'" menu-align="start">
                      <el-button>
                        <span>操作</span>
...
export default {
  ...
  data: function () {
    return {
      ...
      // 转租数据
      rents: ['全部', '转租待确认', '转租费用未结清', '转租费用已结清', '已转租'],
      activeName: '全部',
      input: '',
      // 列表数据
      listData: [],
  ...
  methods: {
    ...
    selected (rent) {
      ...
      if (rent === '全部') {
        this.bean.leaseStatus = 'SUBLET'
      } else if (rent === '转租待确认') {
        this.bean.leaseStatus = 'WASUBLET'
      } else if (rent === '转租已确认') {
        this.bean.leaseStatus = 'ALCONSUB'
      } else if (rent === '转租费用未结清') {
        this.bean.leaseStatus = 'NSUBLET'
      } else if (rent === '转租费用已结清') {
        this.bean.leaseStatus = 'ASUBLET'
      } else if (rent === '已转租') {
        this.bean.leaseStatus = 'SUEND'
      }
      this.loadData(this.bean)
    },
    // 当前页码改变触发
  ...
  created () {
    ...
    // 加载出第一次加载的城市
    this.loadFirst()
    this.$store.state.adminleftnavnum = '/rent/sublet'
