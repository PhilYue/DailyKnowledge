...
=======
        ...
        <el-tab-pane label="密码记录" name="second">
          ...
          <div>
            <el-button @click="deleteAll" v-if="pageInfo.pageTotal > 0 && role === 0" type="text" size="small" class="deleteAll">删除锁上全部密码</el-button>
            <el-pagination
  ...
  methods: {
    ...
    handleClick (tab, event) {
      if (tab.name === 'second') {
        ...
        let houseId = this.form.houseList.length ? this.form.houseList[roomIndex].houseSpaceId : houseId = this.form.houseId
