  ...
  <div>
    <div class="areaSelect">
      <el-breadcrumb separator=">">
        <el-breadcrumb-item v-for="(item, index) in areaData" @click.native="select(item.id, item.name, index)">{{item.name}}</el-breadcrumb-item>
  	  </el-breadcrumb>
    </div>
  	<div class="areaSelect">
      <span v-if="selectData" class="select-span">区域:</span>
      <span class="select-span" v-for="(item, index) in selectData" @click="select2(item.id, item.name, index)">{{item.name}}</span>
  	</div>
  </div>
</template>
<script>
