        ...
        <el-row v-if="contractFormData.earnestShow">
          <el-col :span="12">
            <el-form-item label="定金" prop="earnest">
              <el-input placeholder="请输入定金" v-model="contractFormData.earnest"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <!-- 是否有偿 -->
...
export default {
  ...
  methods: {
    ...
    submitForm (contractFormData) {
      this.$refs[contractFormData].validate((valid) => {
        if (valid) {
          ...
          // 判断是否从流程进来
          if (this.contractFormData.task) {
            let workflowParams
            for (let i = 0; i <= this.options.length - 1; i++) {
              if (this.options[i].actionTarget === 'SUCCESS') {
                workflowParams = this.options[i].workflowParams
              }
            }
            // 流程中token
            this.$store.commit('getMidWorkflowToken', {'processKey': this.contractFormData.processBusinessKey, 'variables': workflowParams})
            // 任务列表进入触发流程
            // 计费规则
            let moneyRule
            // 有首月减不是顾寓
            if (this.contractFormData.firstMoney && this.contractFormData.rentType2 !== '顾寓不规则计费') {
              moneyRule = [
                {
                ...
                // 第一个月的计费规则
                {
                  ...
                  startChargTime: Date.parse(this.contractFormData.startTime),
                  addCycle: 'NORISE',
                  rentFreeStart: null,          // 免租期时间开始
                  rentFreeEnd: null,          // 免租期时间结束
                  cycle: 'ONEMON' // 支付周期
                }
              ]
            } else if (!this.contractFormData.firstMoney && this.contractFormData.rentType2 !== '顾寓不规则计费') { // 无首月减不是顾寓
              moneyRule = [
                {
                  addAmount: this.contractFormData.addAmount * 1000,
                  addPer: this.contractFormData.addPer * 1000,
            ...
            } else if (!this.contractFormData.firstMoney && this.contractFormData.rentType2 === '顾寓不规则计费') {  // 无首月减是顾寓
              moneyRule = [
              // 前三个月计费规则
                {
                  addAmount: null,
                  ...
                  financeType: 'INC',
                  addType: 'NOTRISE',
                  endChargTime: this.guyuthreeMonthEnd,
                  initialAmount: this.contractFormData.rentMoney * 1000,
                  startChargTime: Date.parse(this.contractFormData.startTime),
                  addCycle: 'NORISE',
                  rentFreeStart: Date.parse(this.contractFormData.rentFreeStart),          // 免租期时间开始
                ...
                // 后9个月的计费规则
                {
                  ...
                  endChargTime: Date.parse(this.contractFormData.endTime),
                  initialAmount: this.contractFormData.rentMoney * 1000,
                  startChargTime: this.guyuthreeMonthEnd + 86400000,
                  addCycle: 'NORISE',
                  rentFreeStart: null,          // 免租期时间开始
                  rentFreeEnd: null,          // 免租期时间结束
                  cycle: 'ONEMON' // 支付周期
            ...
            } else if (this.contractFormData.firstMoney && this.contractFormData.rentType2 === '顾寓不规则计费') {  // 有首月减是顾寓
              moneyRule = [
              // 首月减录进去抵扣
                {
                  addAmount: null,
                  ...
                  financeType: 'DISC',
                  addType: 'NOTRISE',
                  endChargTime: this.guyufirstMonthEnd,
                  initialAmount: this.contractFormData.firstMoney * 1000,
                  startChargTime: Date.parse(this.contractFormData.startTime),
                  addCycle: 'NORISE',
                  rentFreeStart: null,          // 免租期时间开始
            ...
            this.$http.post(this.$store.state.contractUrl + 'addLeaContract', {
              ...
              storageType: this.contractFormData.storageType, // 电子合同还是纸质合同
              ...
              certificateNum: this.contractFormData.cardId,              // 证件号码
              certificateType: this.contractFormData.certificateType,  // 证件类型
              ...
              memo: this.contractFormData.memo,  // 备注
              ...
              headers: {
                ...
                'processInstanceId': this.contractFormData.processInstanceId,
                'taskId': this.contractFormData.taskId,
                'userId': this.$store.state.currentUser.uid,
                'processBusinessKey': this.contractFormData.processBusinessKey
              }
            }).then(function (res) {
              var data = res.data
              if (data.code === 200) {
    ...
    // 顾寓时间选择
    guyuMoneyMonth () {
      if (this.contractFormData.startTime && this.contractFormData.endTime && this.contractFormData.rentType2 === '顾寓不规则计费') {
        // 拿第一个月的时间
        this.$http.post(this.$store.state.contractUrl + 'getTime', {
          nowTime: Date.parse(this.contractFormData.startTime),
          index: 1
          ...
          if (data.data !== null) {
            ...
            console.log(this.guyufirstMonthEnd)
          }
        }, function (response) {
          console.log(response.status)
        })
        ...
        this.$http.post(this.$store.state.contractUrl + 'getTime', {
          ...
          if (data.data !== null) {
            ...
            console.log(this.guyuthreeMonthEnd)
          }
        }, function (response) {
          console.log(response.status)
        })
    ...
    changeTime (val) {
      ...
      if (this.contractFormData.rentType2 === '顾寓不规则计费') {
        this.guyuMoneyMonth()
      }
      let v
  ...
  props: ['contractForm', 'contractFormData', 'options'],
  mounted () {
  ...
  watch: {
    ...
    'contractForm.show': function (val, oldVal) {
      ...
      console.log(this.contractFormData.storageType)
      // this.resetForm('contractFormData')
    }
  }
}
