        ...
        <el-row>
          ...
          <el-col :span="2">
            ...
            <div v-else class="pad8">
              <div v-show="yqShow">
                ...
                <div v-if="item.orderStatus === 'NEW'">
                  <p class="orange" v-if="7 >= dateDiff(new Date(item.newPayDeadlineTime).toLocaleDateString(),new Date(new Date().getTime()).toLocaleDateString()) > 0 && dateDiff(new Date(item.newPayDeadlineTime).toLocaleDateString(),new Date(new Date().getTime()).toLocaleDateString()) !== 0">逾期{{dateDiff(new Date(item.newPayDeadlineTime).toLocaleDateString(),new Date(new Date().getTime()).toLocaleDateString())}}天</p>
          ...
          <el-col :span="3">
            <div class="pad8" v-if="item.payTime">
               ...
               <p v-else-if="item.orderStatus === 'SUC' && item.distributeFlag === false">支付成功
               <span v-if="item.payChannelType">({{item.payChannelType | payType}})</span></p>
               <p v-else-if="item.orderStatus === 'SUC' && item.distributeFlag === true">支付成功(待分账)
               <span v-if="item.payChannelType">({{item.payChannelType | payType}})</span></p>
               <p v-else-if="item.orderStatus === 'FAIL'">支付失败</p>
               <p v-else-if="item.orderStatus === 'PDOFF' && item.distributeFlag === false">线下已支付
               <span v-if="item.payChannelType">({{item.payChannelType | payType}})</span></p>
               <p v-else-if="item.orderStatus === 'PDOFF' && item.distributeFlag === true">线下已支付(待分账)
               <span v-if="item.payChannelType">({{item.payChannelType | payType}})</span></p>
               <p v-else-if="item.orderStatus === 'PDON' && item.distributeFlag === false">线上直接转账已支付
               <span v-if="item.payChannelType">({{item.payChannelType | payType}})</span></p>
               <p v-else-if="item.orderStatus === 'PDON' && item.distributeFlag === true">线上直接转账已支付(待分账)
               <span v-if="item.payChannelType">({{item.payChannelType | payType}})</span></p>
          ...
          <el-col :span="2">
            <div class="pad8">
              <p>
                <span v-if="item.payPurposeType === 'REN'">租金.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'WATER'">水费.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'ELEC'">电费.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'GAS'">煤气费.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'COM'">赔偿费.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'CLEAN'">清洁费.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'MAIN'">维修费.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'CONSUME'">消费.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'OTH'">其他.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'PERFEE'">物管费.{{item.periodNumber}}期</span>
                <span v-else-if="item.payPurposeType === 'MANAGECOST'">手续费.{{item.periodNumber}}期</span>
