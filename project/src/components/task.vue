<template>
  <div>
        ...
        <!-- 合同期间和房源地址1，2 -->
          ...
          <el-col :span="3" class="pad9">
            <el-dropdown menu-align="start">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="(item,index2) in taskData[index1].operation.options" @click.native="operation(index1, item.hint, item.actionUrl, item.requestMethod, item.actionType, item.workflowParams, item.actionTarget, item.businessParams, item.options)">{{item.hint}}</el-dropdown-item>
    ...
    <!-- 新建合同表单组件 -->
    <contractForm :options="options" :contractForm="contractForm" :contractFormData="contractFormData"></contractForm>
    <!-- 维修反馈 -->
    ...
    <!--租约和合同并存的详情 -->
    <taskDetail :taskDetailShow="taskDetailShow" :detailData="detailData" :detailDataN="detailDataN"></taskDetail>
    <!-- 合同详情组件 -->
    <contractDetail :detailShow="detailShow" :contractId="contractId"></contractDetail>
  </div>
</template>
<script>
import $ from 'jquery'
...
export default {
  ...
  methods: {
    // 修改状态统一接口POST
    changStatus (apiMethod, url, data, processBusinessKey) {
          ...
          callback: (action, instance) => {
            ...
            if (action === 'confirm') {
              ...
              if (req.processBusinessType === 'maintain') {
                // 维保的链接
                apiUrl = 'http://192.168.1.11:23300' + actionUrl
              } else if (req.processBusinessType === 'rent') {
                // 新签合同链接
                apiUrl = 'http://192.168.1.11:23250' + actionUrl
              } else if (req.processBusinessType === 'renewal' && actionTarget === 'confirm.rent.contract.workflow') {
                // 续租第一步审核链接
                apiUrl = 'http://192.168.1.11:21600' + actionUrl
                ...
                // 退租第二步链接
                apiUrl = 'http://192.168.1.11:23150' + actionUrl
              }
              // 如果是合同同意取消
              if (requestMethod === 'POST') {
                ...
                this.changStatus(requestMethod, apiUrl, apiData, req.processBusinessKey)
    ...
    openDetails ($event, index1) {
      if ($event.target.innerHTML === '操作') {
        ...
        if (this.taskData[index1].processBusinessType === 'maintain') {
        ...
        } else if (this.taskData[index1].processBusinessType === 'rent' || this.taskData[index1].processBusinessType === 'rent.terminate') { // 打开合同详情
          this.contractId = this.taskData[index1].variables.id
          ...
          // 打开续租转租合同和老租约详情
          this.$http.get(this.$store.state.contractUrl + 'getContractDetailsById/' + this.taskData[index1].variables.id, {
          }).then(function (response) {
            var data = response.data
            if (data.code === 200) {
              ...
              this.$http.get(this.$store.state.contractUrl + 'getContractDetailsById/' + data.data.originalContractId, {
              }).then(function (response) {
                var data = response.data
                if (data.code === 200) {
                  this.detailDataN = data.data
                  this.taskDetailShow.show = true
                  for (var j = 0; j <= this.detailDataN.users.length - 1; j++) {
                    if (this.detailDataN.users[j].representative === 'PARTB') {
                      this.detailDataN.userName = this.detailDataN.users[j].userName
                      this.detailDataN.userPhone = this.detailDataN.users[j].userPhone
                      this.detailDataN.idCard = this.detailDataN.users[j].certificateNumber
                      this.detailDataN.sex = this.detailDataN.users[j].sex
                    } else if (this.detailDataN.users[j].representative === 'PARTA') {
                      this.detailDataN.houseKeeperName = this.detailDataN.users[j].userName
                    }
                  }
                }
              }, function (response) {
