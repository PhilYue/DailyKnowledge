      ...
      <!-- 姓名手机号 -->
        <el-form-item label="" prop="changeType">
          <el-radio-group @change="getPay" v-model="huanRentForm.changeType">
            <el-radio class="radio" label="CPAID">有偿换房</el-radio>
            <el-radio class="radio" label="CNOTPAID">无偿换房</el-radio>
          </el-radio-group>
        </el-form-item>
...
<script>
export default {
  props: ['huanRentFormData', 'huanRentFormShow', 'huanRentForm'],
  data: function () {
    return {
      tableData: [], // 财务表格数据
      changeType: '',
      ...
      this.$refs[huanRentForm].validate((valid) => {
        if (valid) {
          ...
          let form = {
            ...
            poundage: this.huanRentForm.changeType,
            expiredOutTime: this.huanRentForm.time,
            startTime: new Date(Date.parse(this.huanRentForm.time) + 86400000),
            endTime: null
          }
